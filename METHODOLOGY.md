...
The bootstrap questions themselves are a template (`templates/SETUP_QUESTIONS.md`, inlined into the prompt): three dependency-ordered rounds, each option with its consequence, a default, the cost of changing later, and the config field it writes. Policies ship empty in `fdc.conf`; an empty field means "not decided", never a default.
